...
export default function DetailNote({ onClose }: { onClose: () => void }) {
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="bg-slate-200 rounded-[20px] px-1.5 py-1 flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-sm font-normal text-slate-700">
                  {todoData?.todos.find((todo) => todo.id)?.file_url ||
                    todoData?.todos.find((todo) => todo.id)?.link_url}
                </span>
              </div>
              <span className="text-base font-normal text-slate-700">
                {data?.notes.map((note) => note.content)}
              </span>
            </div>
          </div>
        </div>
